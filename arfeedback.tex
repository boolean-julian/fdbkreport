...
\begin{document}
	...
	\section*{Summary and Outlook}\label{sec:summary}
		This paper aims at contributing towards improving the overall mobile AR experience of users by 
		providing them the appropriate feedback in case of usage of unsupported gestures. We developed 
		3 different kinds of feedbacks and conducted a case study to test them with users which were 
		divided into 4 different groups. Users in different groups received different type of feedbacks. 
		The \nameref{sec:approach} and \nameref{sec:casestudy} sections described in detail the different feedback types and how 
		users were divided into different groups. 

		Our results show that users are more tempted to use incorrect gesture inputs with the second task 
		of our Toaster prototype. These two gestures are two-finger rotation and pinch-zoom. Hence we decided 
		to compare two different parameters for this specific task - time the users needed to complete the task 
		with the aid of different feedback types and the number of wrong gesture inputs. Supportive feedback type 
		seems to be most successful to guide the user than other feedback types. Users who received supportive 
		feedback needed 2 times less time to complete the task and used 3 to 4 times less wrong gesture inputs 
		in comparison to other groups. 

		In the future, we will conduct another case study to test other wrong usages by using different tasks. 
		We will also consider other feedback types with different visibility and content. 
